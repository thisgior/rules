...
步进 0、步进 1、步进 2 与步进 3 已完成。下一开发动作只执行“步进 4”：Clash/Mihomo 编译器、classical/domain payload、规则引用片段及通用域名列表。
...
### 24.3 步进 2 实施记录（2026-08-21）

- [x] 建立普通 `Rule`、`ParseIssue` 与 `ParseResult` 数据模型。
- [x] 解析普通域名、`*.` 与前导 `.` 后缀域名。
- [x] 解析 HTTP/HTTPS URL，并且只保留 hostname。
- [x] 解析 `DOMAIN`、`DOMAIN-SUFFIX`、`DOMAIN-KEYWORD`。
- [x] 显式规则默认保留原策略，`--override-policy` 才覆盖。
- [x] Unicode 域名规范化为 IDNA/Punycode，同时保留原 Unicode 值。
- [x] 域名转小写、移除末尾根点并校验标签长度和字符。
- [x] 自动跳过同策略完全重复项，并报告首次出现行号。
- [x] 相同匹配指向不同策略时不静默删除，保留并报告策略冲突。
- [x] 非法 URL、中间通配符、IP/CIDR、无协议路径和缺失字段逐行报错。
- [x] URL 用户名和密码在进入报告与模型前遮盖。
- [x] 支持 UTF-8 文件和标准输入，限制输入文件最大 8 MiB。
- [x] `parse-rules` 支持中文文本与 JSON 输出。
- [x] 有解析错误时稳定返回退出码 2，文件错误返回退出码 5。
- [x] 版本更新为 `0.1.0.dev2`。
- [x] 全部 30 项自动测试通过。
- [x] Python 3.9 语法静态检查和 Bash 语法检查通过。

步进 2 完成时仍只解析并输出结果；`sources/` 与规则项目写入从步进 3 开始提供，代理配置始终未被修改，也未重载服务。

### 24.4 步进 3 实施记录（2026-08-21）

- [x] 建立版本化的 `sources/<source-id>.yaml` 通用规则项目格式。
- [x] 保存来源 ID、类型、标签和可选位置，以及规则的稳定 ID、策略、原始输入、启用状态、优先级和创建时间。
- [x] 实现 `init-project`、`add-rules`、`list-rules`、`delete-rule`、`enable-rule` 与 `disable-rule` 子命令。
- [x] Bash 中文菜单加入项目创建、规则添加与规则列出入口；写入前明确确认。
- [x] 输入仍有解析错误时整批不写入。
- [x] 跨来源完全重复自动跳过并报告原规则 ID。
- [x] 匹配相同但策略不同的规则同时保留，报告冲突且不覆盖原规则。
- [x] 来源 ID 使用白名单校验，阻止路径逃逸；URL 来源位置拒绝嵌入用户名或密码。
- [x] 读取时校验 schema、来源元数据、规则字段、优先级与全项目规则 ID 唯一性。
- [x] 写入前重新解析生成的 YAML，使用同目录临时文件、`fsync`、权限保持和原子替换。
- [x] 版本更新为 `0.1.0.dev3`。
- [x] 全部 42 项自动测试通过。
- [x] Python 3.9 语法静态检查和 Bash 语法检查通过。

当前只写入用户明确指定的通用规则项目，不编译 Clash/Mihomo 产物，不修改任何代理配置，也不重载服务。当前运行环境仍为 Ubuntu 24.04 / Python 3.12，真实 Debian 11–13 容器矩阵尚未执行。
